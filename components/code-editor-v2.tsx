...
import { Expand, Play, Send, Loader2, Minimize2, CheckCircle, XCircle, Circle } from "lucide-react";
...
import { LANGUAGE_CONFIGS, SupportedLanguage } from "@/lib/code-execution/types";
    ...
    const runCode = async (
        ...
        try {
            ...
            if (result.success) {
                ...
                return {
                    success: false,
                    output: result.stderr || result.compileOutput || result.error || "Error",
                };
            }
        } catch (error) {
            return {
                                ...
                                <div className="problem-markdown">
                                    <Markdown
                                        components={{
                                            ...
                                            h2: ({ children }) => <h2 className="text-lg font-semibold text-primary mt-5 mb-2">{children}</h2>,
                                            ...
                                            ul: ({ children }) => <ul className="list-disc list-inside text-muted-foreground mb-3 space-y-1 ml-2">{children}</ul>,
                                            ol: ({ children }) => <ol className="list-decimal list-inside text-muted-foreground mb-3 space-y-1 ml-2">{children}</ol>,
                                            ...
                                            pre: ({ children }) => <pre className="bg-card border border-border p-4 rounded-lg overflow-x-auto my-3 font-mono text-sm text-foreground">{children}</pre>,
                                            strong: ({ children }) => <strong className="text-foreground font-semibold">{children}</strong>,
                                            em: ({ children }) => <em className="text-muted-foreground italic">{children}</em>,
                                            blockquote: ({ children }) => <blockquote className="border-l-4 border-primary pl-4 my-3 text-muted-foreground italic bg-muted/30 py-2 rounded-r">{children}</blockquote>,
                                        }}
                                    >
                                        {problem?.description}
                                    </Markdown>
                                                    ...
                                                    <ResizablePanel defaultSize={50} className="overflow-auto">
                                                        <div className="h-full flex flex-col">
                                                            ...
                                                            <div className="p-4 font-mono text-sm overflow-auto flex-1">
                                                                {localOutput ? (
                                                                    <pre className="text-foreground">{localOutput}</pre>
                                                                ) : (
                                            ...
                                            >
                                                {stream.submissionId ? (
                                                    ...
                                                    <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
                                                        ...
                                                        <p className="text-lg">Submit your code to see results</p>
                                                        <p className="text-sm mt-2">
                                                            Your submission will be tested against all test cases
                                                        </p>
                                                    </div>
                                                )}
                                            </TabsContent>
