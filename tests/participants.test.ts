...
import { POST, GET, DELETE } from '@/app/api/orgs/[orgId]/contests/[contestId]/participants/route';
...
jest.mock('@/app/api/orgs/[orgId]/contests/[contestId]/participants/service', () => ({
...
describe('POST /participants', () => {
  ...
  it('should register a participant successfully', async () => {
    ...
    (participantService.registerParticipant as jest.Mock).mockResolvedValue({ email: mockEmail });
  ...
  it('should return 400 on Zod validation error', async () => {
    ...
    const res = await POST(req, { params: { orgId: 'invalid org!', contestId: 'bad/id' } });
  ...
  it('should return 403 for disallowed errors', async () => {
    (getOrgIdFromNameId as jest.Mock).mockResolvedValue(mockOrgId);
    (getContestIdFromNameId as jest.Mock).mockResolvedValue(mockContestId);
    (participantService.registerParticipant as jest.Mock).mockRejectedValue(new Error('User already registered'));
...
describe('GET /participants', () => {
  ...
  it('should return participants successfully', async () => {
    (getOrgIdFromNameId as jest.Mock).mockResolvedValue(mockOrgId);
    (getContestIdFromNameId as jest.Mock).mockResolvedValue(mockContestId);
    (participantService.getContestParticipants as jest.Mock).mockResolvedValue([{ email: mockEmail }]);

    const res = await GET({} as unknown as NextRequest, { params: validParams });
    expect(res.status).toBe(200);
    expect(res.data).toEqual([{ email: mockEmail }]);
  });

  it('should return 404 for not found errors', async () => {
    (getOrgIdFromNameId as jest.Mock).mockRejectedValue(new Error('Organization not found'));
    const res = await GET({} as unknown as NextRequest, { params: validParams });
...
describe('DELETE /participants', () => {
  ...
  it('should remove participant successfully', async () => {
    (getOrgIdFromNameId as jest.Mock).mockResolvedValue(mockOrgId);
    (getContestIdFromNameId as jest.Mock).mockResolvedValue(mockContestId);
    (participantService.removeParticipant as jest.Mock).mockResolvedValue(undefined);
  ...
  it('should return 404 if user not registered', async () => {
    (getOrgIdFromNameId as jest.Mock).mockResolvedValue(mockOrgId);
    (getContestIdFromNameId as jest.Mock).mockResolvedValue(mockContestId);
    (participantService.removeParticipant as jest.Mock).mockRejectedValue(new Error('User not registered'));
