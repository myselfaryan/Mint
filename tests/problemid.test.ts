...
import { GET, PATCH, DELETE } from '@/app/api/orgs/[orgId]/problems/[problemId]/route';
...
const createMockRequest = (body: any): NextRequest => ({ json: async () => body } as unknown as NextRequest);
...
describe('GET /problems/:problemId', () => {
  ...
  it('should return 404 if not found', async () => {
    (getOrgIdFromNameId as jest.Mock).mockResolvedValue(mockOrgId);
    (problemService.getProblem as jest.Mock).mockRejectedValue(new Error('Problem not found'));
...
describe('PATCH /problems/:problemId', () => {
  ...
  it('should update problem successfully', async () => {
    ...
    (getOrgIdFromNameId as jest.Mock).mockResolvedValue(mockOrgId);
    (problemService.updateProblem as jest.Mock).mockResolvedValue({ ...mockProblem, ...updateData });
  ...
  it('should return 404 if problem not found during update', async () => {
    (getOrgIdFromNameId as jest.Mock).mockResolvedValue(mockOrgId);
    (problemService.updateProblem as jest.Mock).mockRejectedValue(new Error('Problem not found'));
...
describe('DELETE /problems/:problemId', () => {
  ...
  it('should return 404 if problem not found', async () => {
    (getOrgIdFromNameId as jest.Mock).mockResolvedValue(mockOrgId);
    (problemService.deleteProblem as jest.Mock).mockRejectedValue(new Error('Problem not found'));
  ...
  it('should return 409 if problem is used in contests', async () => {
    (getOrgIdFromNameId as jest.Mock).mockResolvedValue(mockOrgId);
    (problemService.deleteProblem as jest.Mock).mockRejectedValue(new Error('Problem is used in contests'));
