...
describe('GET /users', () => {
  ...
  it('should return 500 if fetching fails', async () => {
    (getOrgIdFromNameId as jest.Mock).mockResolvedValue(mockOrgId);
    (userService.getOrgUsers as jest.Mock).mockRejectedValue(new Error('DB error'));
...
describe('POST /users', () => {
  ...
  it('should invite user successfully', async () => {
    const req = createMockRequest({ email: 'user@example.com', role: 'member' });
    (getOrgIdFromNameId as jest.Mock).mockResolvedValue(mockOrgId);
  ...
  it('should return 400 for service errors', async () => {
    const req = createMockRequest({ email: 'user@example.com', role: 'member' });
    (getOrgIdFromNameId as jest.Mock).mockResolvedValue(mockOrgId);
    (userService.inviteUser as jest.Mock).mockRejectedValue(new Error('User already exists'));
