...
describe('GET /submissions', () => {
  ...
  it('should return submissions successfully', async () => {
    ...
    (submissionService.getOrgSubmissions as jest.Mock).mockResolvedValue([mockSubmission]);
...
describe('POST /submissions', () => {
  ...
  it('should return 400 for known error thrown by service', async () => {
    (getOrgIdFromNameId as jest.Mock).mockResolvedValue(mockOrgId);
    (submissionService.createSubmission as jest.Mock).mockRejectedValue(new Error('Invalid contest'));
...
describe('GET /submissions/[submissionId]', () => {
  ...
  it('should return a single submission successfully', async () => {
    ...
    (submissionService.getSubmission as jest.Mock).mockResolvedValue(mockSubmission);

    const res = await import('@/app/api/orgs/[orgId]/submissions/[submissionId]/route').then(m => m.GET({} as unknown as NextRequest, { params }));
    expect(res.status).toBe(200);
    expect(res.data).toEqual(mockSubmission);
  });
